...
(function (b) {
    var a = {
        ...
        methods: {
            setup: function (f, i) {
                var c = b(this) || b(),
                    f = b.extend({}, a.defaults, f),
                    h = c.attr("data-" + f.namespace + "-variant") || f.variant,
                    g = !f.resetCss ? f.namespace : f.namespace + "-reset",
                    e = b(f.background || '<div class="' + g + '"><div class="' + g + '-content"><span class="' + g + '-close">X</span></div></div>'),
                    d = {
                        config: f,
                        content: i,
                        $elm: c,
            ...
            getContent: function () {
                var d = this,
                    f = d.content,
                    c = d.$elm.attr(d.config.targetAttr);
                if (typeof f === "string") {
                    d.content = b(f);
                } else {
                    if (f instanceof b === false) {
                        if (c === "image" || c.match(/\.(png|jpg|jpeg|gif|tiff|bmp)$/i)) {
                            var e = c.match(/\.(png|jpg|jpeg|gif|tiff|bmp)$/i) ? c : d.$elm.attr("href");
                            d.content = b('<img src="' + e + '" alt="" class="' + d.config.namespace + '-image" />');
                        } else {
                            if (c === "ajax" || c.match(/(http|htm|php)/i)) {
                                var e = c.match(/(http|htm|php)/i) ? c : d.$elm.attr("href"),
                                    f = e ? b("<div></div>").load(e, function (h, g) {
                                        if (g !== "error") {
                ...
                } if (d.content.is("iframe") || b("iframe", d.content).length > 0) {
            ...
            close: function (e) {
                ...
                var c = this,
                    d = c.config,
                    f = b(e.target);
                if ((d.clickBgClose && f.is("." + d.namespace)) || f.is("." + d.namespace + "-close")) {
